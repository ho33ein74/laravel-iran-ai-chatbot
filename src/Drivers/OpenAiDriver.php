...
class OpenAiDriver implements AiDriverInterface {
    ...
    public function ask(string $prompt, array $history = []): string {
        $response = Http::withToken($this->config['api_key'])->timeout(60)->post($this->config['endpoint'], ['model' => $this->config['model'] ?? 'gpt-4o-mini', 'messages' => array_merge($history, [['role' => 'user', 'content' => $prompt]])]);
        if ($response->failed()) throw new \Exception("OpenAI API Error");
