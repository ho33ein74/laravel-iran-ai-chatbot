...
یک پکیج جامع، سبک و حرفه‌ای برای راه‌اندازی دستیار هوشمند و چت‌بات در پروژه‌های لاراولی. این پکیج از سرویس‌دهنده‌های مختلف (AvalAI، GapGPT، OpenAI، Gemini و مدل‌های Local)، جستجوی آفلاین در دیتابیس (RAG بومی)، و سیستم احراز هویت هوشمند پشتیبانی می‌کند.

## ویژگی‌های کلیدی
*   🚀 **معماری بدون دیتابیس برای تنظیمات (Zero DB Settings):** تمام تنظیمات پایه‌ای از طریق فایل `.env` و ظاهر چت‌بات مستقیماً توسط `Props` در فرانت‌اند کنترل می‌شود تا سرعت و یکپارچگی به حداکثر برسد.
*   **پشتیبانی از چندین درایور هوش مصنوعی:** AvalAI, GapGPT, OpenAI, Gemini و مدل‌های لوکال (با قابلیت انتخاب مدل داینامیک).
*   **ظاهر کاربری (UI) پیشرفته و مستقل:** کامپوننت Vue.js اختصاصی با قابلیت شخصی‌سازی کامل رنگ‌ها، متون، نحوه نمایش (پاپ‌آپ، سایدبار، تمام‌صفحه) و بدون وابستگی به استایل‌های قالب اصلی سایت.
*   **جستجوی مستقیم دیتابیس (Native RAG):** پیدا کردن محصولات و مقالات سایت و پیشنهاد آن‌ها به کاربر به صورت **اسلایدرهای افقی و عکس‌دار (Carousel)** بدون نیاز به مصرف توکن API.
...
## ۲. تنظیمات فایل `.env` (پیکربندی API)

تمام تنظیمات حیاتی سیستم، کلیدهای API و محدودیت‌ها فقط از طریق فایل `.env` پروژه شما انجام می‌شود:

```env
# درایور پیش‌فرض (gapgpt, avalai, openai, gemini, local)
IRAN_AI_DEFAULT_DRIVER=gapgpt

# تنظیمات مربوط به GapGPT
...
برای اینکه ربات بتواند در محصولات شما جستجو کند و نتایج را به صورت کارت‌های قابل کلیک نمایش دهد، فایل `config/iran-ai-chatbot.php` را باز کنید و بخش `searchable_models` را به شکل زیر مقداردهی کنید:
...
ویجت چت‌بات به عنوان یک کامپوننت Vue طراحی شده است. تمام ظاهر، متون و پیام‌ها **مستقیماً از طریق پراپ‌ها (Props)** قابل شخصی‌سازی است و نیازی به هیچ جدول تنظیماتی در دیتابیس ندارد.
...
| --- | --- | --- | --- |
| `color` | String | `#1a56db` | رنگ اصلی چت‌بات (هدر، دکمه‌ها و...) |
| `title` | String | `دستیار هوشمند` | عنوان بالای چت‌بات |
...
* **مدیریت مهمانان:** مکالمات مهمانان با استفاده از شناسه‌های امن کوکی (`session_id`) مدیریت و ذخیره می‌شوند.
* **مدیریت کاربران لاگین شده:** در صورت تشخیص لاگین بودن کاربر (چه از طریق `web` و چه `sanctum`/`api`)، پیام‌ها مستقیماً با `user_id` آن‌ها ذخیره می‌شود تا در دستگاه‌های دیگر نیز به تاریخچه خود دسترسی داشته باشند.
